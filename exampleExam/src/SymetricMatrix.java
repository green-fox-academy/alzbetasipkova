public class SymetricMatrix {
    public static void main(String[] args) {
        ...
        int[][] anotherSymmetricMatrix = {
                {6, 7, 8},
                {2, 5, 7},
                {1, 2, 6}
        };

        System.out.println("Testing is symetric");
        ...
        System.out.println("Testing another is symetric");
        ...
        for (int i = 0; i < inputMatrix.length; i++) {
            for (int j = 0; j < inputMatrix[i].length; j++) {
                if (inputMatrix[i][j] != inputMatrix[j][i]) {
                    System.out.println("the firts index when matrix isnt symetric" + i + " and "+ j);
                    return false;
                }
            }
        }
